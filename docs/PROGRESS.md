...
- **プレビュー別ポート時の CSS 404/接続拒否を回避**
  - `templates/editor.html` に `editor.css` のフォールバック読込ロジックを追加。
    - 既定の `/static/css/editor.css?v=1.0.1` が適用されない/読み込み失敗時、`127.0.0.1:8000` から同 CSS を再取得して適用。
    - onerror ハンドラと `document.styleSheets` 遅延確認の二段構えで適用可否を判定。
  - テスト手順を `TESTING.md` に追記（ネットワークタブ/コンソールでの確認項目を追加）。
  - これにより、IDE のプレビュー機能など、バックエンド（8000番）とは異なるポートで `/editor` を開いてもスタイルが崩れないことを確認。

- **シーンリンクのナビゲーション対応**
  - `static/js/editor.js` にグローバル `window.NovelGenPage.navigateToScene(sceneId, sceneName)` を実装。
    - プレビュー内 `.scene-link` クリック時、対応する `(scene:<id>)` の最初の位置へ Quill のカーソル移動＋フォーカス。
    - 併せてプレビュー領域を上部へスムーズスクロール（今後 h2 アンカー移動へ拡張予定）。
  - これにより、従来表示されていた「navigateToScene ハンドラが設定されていません」警告は解消。
  - `TESTING.md` に検証項目（警告が出ないこと/カーソル移動/プレビューのスクロール）を追加。
